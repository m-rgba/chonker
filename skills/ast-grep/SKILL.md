...
- **Structural Search**: Finding code patterns (e.g., `try { ... } catch`) regardless of formatting.
- **Refactoring**: Safely replacing code patterns (e.g., changing function signatures).
- **Linting**: Enforcing codebase-specific rules.

## When not to use this

- **Simple Text Search**: Use **ripgrep** (rg). If you are looking for a variable name or error string, `rg` is faster and easier.
- **File Search**: Use **fd**. `ast-grep` searches code content, not filenames.
