---
name: browser-use
description: This skill should be used when users require advanced browser automation capabilities, such as managing complex persistent state, running Python scripts within the browser session, or using cloud/real-user profiles. It serves as an alternative to agent-browser for complex or specialized workflows.
...
Use `browser-use` when `agent-browser` capabilities are insufficient or when specific features are required:

- **Complex State**: When you need heavy state persistence specific to `browser-use` features.
- **Python Scripting**: If you need to run Python code within the browser interaction loop (`browser-use python`).
- **Cloud/Real Profiles**: When using specific `browser-use` cloud or real-browser profile features.
- **Advanced SPAs**: If the target site is significantly easier to navigate with `browser-use`'s index-based approach.

**Note**: For standard tasks, prefer `agent-browser` for better token efficiency.
...
| ------------------ | --------------------------------- | ------------------------ |
| **Token Usage**    | Higher                            | **Lower (Default)**      |
| **Identification** | Indices (`1`, `2`)                | Refs (`@e1`)             |
| **Scripting**      | **Python Support**                | Bash-centric             |
| **Best For**       | Complex flows, Python integration | Standard tasks, scraping |
